...
const Editor = ({ value }) => {
  ...
  const renderElement = React.useCallback(({ attributes, children, element }) => {
    switch (element.type) {
      ...
      case 'relationship':
        return <Image className="rounded-lg shadow-md overflow-hidden" src={element.data.data.file.url} alt={element.data.label} width={367} height={275} loading="eager" priority />
      default:        
        return <></>
    }
  }, [])
